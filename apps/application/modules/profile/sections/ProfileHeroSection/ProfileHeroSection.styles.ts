...
export const profileHeroSectionStyles = tv({
  slots: {
    ...
    notch: "hidden",
    backButton:
      "absolute end-5 top-[calc(env(safe-area-inset-top)+1rem)] z-[2] bg-background/72 backdrop-blur-xl",
    overlap:
      "relative z-[2] mt-24 flex items-center justify-between px-[max(1.25rem,calc(18.5%-1.5rem))]",
    sideButton:
      "!size-12 !min-w-12 !rounded-[1.1rem] bg-background [&_.icon]:!text-[18px]",
    avatarLink:
    ...
    avatarBadge:
      "size-8 min-w-8 bg-foreground text-background",
    identity:
