...
type Level =
  "countryId" | "provinceId" | "cityId" | "districtId" | "cityRegionId";
...
export function LocationFormScreen({ role, locationId }: Props) {
  ...
  const provinces = usePublicCatalogResource(
    "location",
    "province",
    { parentId: selection.countryId, limit: LIMIT },
    Boolean(selection.countryId),
  );
  const cities = usePublicCatalogResource(
    "location",
    "city",
    { parentId: selection.provinceId, limit: LIMIT },
    Boolean(selection.provinceId),
  );
  const districts = usePublicCatalogResource(
    "location",
    "district",
    { parentId: selection.cityId, limit: LIMIT },
    Boolean(selection.cityId),
  );
  const cityRegions = usePublicCatalogResource(
    "location",
    "city-region",
    { parentId: selection.cityId, limit: LIMIT },
    Boolean(selection.cityId),
  ...
  const itemName = (level: Level) =>
    selectors[level].items.find((item) => item.id === selection[level])?.name;
  ...
  function selectLocation(level: Level, id: string) {
    setSelectionOverride((override) => {
      ...
      if (level === "provinceId")
        return { ...EMPTY, countryId: current.countryId, provinceId: id };
      if (level === "cityId")
        return { ...current, cityId: id, districtId: "", cityRegionId: "" };
  ...
  return (
    <main className="app-page">
          ...
          {(Object.keys(selectors) as Level[]).map((level) => {
            ...
            return (
              <TextField
                ...
                isDisabled={selector.disabled}
              >
                <Label>{selector.label}</Label>
                <InputGroup
                  variant="secondary"
                  onClick={() => !selector.disabled && setOpenLevel(level)}
                >
                  <InputGroup.Input
                    readOnly
                    value={itemName(level) ?? ""}
      ...
      {openLevel ? (
        <LocationPickerSheet
          key={openLevel}
          selectedId={selection[openLevel]}
          {...selectors[openLevel]}
          onClose={() => setOpenLevel(null)}
          onSelect={(id) => selectLocation(openLevel, id)}
        />
      ) : null}
    </main>
  );
  ...
  return (
    <TextField {...props}>
      ...
      <InputGroup variant="secondary">
        {multiline ? <InputGroup.TextArea rows={3} /> : <InputGroup.Input />}
      </InputGroup>
    </TextField>
  );
}
...
function LocationPickerSheet({
  ...
  return (
    <BottomSheet
      ...
      snapPoints={[0.62, 0.9]}
      title={`انتخاب ${label}`}
      description={`یک ${label} را از فهرست انتخاب کنید.`}
    >
      <TextField
        value={search}
        onChange={setSearch}
        aria-label={`جستجوی ${label}`}
        className="sticky top-0 z-10 bg-surface pb-3"
      >
      ...
      {isLoading ? (
        <PickerRowsSkeleton count={5} />
      ) : isError ? (
        <div className="flex min-h-40 flex-col items-center justify-center gap-3 text-center">
          <Typography type="body-sm" className="text-danger">
            دریافت فهرست ناموفق بود.
          </Typography>
