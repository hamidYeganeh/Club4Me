...
export const discoveryClubSlotsScreenStyles = tv({
  slots: {
    ...
    chipActive: "bg-accent text-accent-foreground",
    chipIdle: "bg-surface/85 text-foreground backdrop-blur-md",
    panel: "relative z-30 mx-3 -mt-6 rounded-[var(--app-radius-feature,24px)] bg-surface p-4 shadow-sm sm:mx-4 sm:p-5",
    panelBody: "space-y-5",
    ...
    dateTab: "group flex min-h-20 min-w-[4.25rem] shrink-0 flex-col items-center justify-center gap-2 rounded-[var(--app-radius-control,16px)] !bg-surface-secondary px-3 py-3 !text-foreground opacity-100 transition-colors data-[selected=true]:!bg-surface data-[selected=true]:!text-foreground focus-visible:outline-2 focus-visible:outline-focus",
    ...
    timeEmpty: "flex min-h-28 w-full flex-col items-center justify-center gap-2 rounded-2xl bg-surface-secondary px-4 py-4 text-center text-sm text-muted",
    timeEmptyImage: "size-12 object-cover",
    timeContent: "flex min-h-12 w-full items-center justify-center rounded-[var(--app-radius-control,16px)] !bg-surface-secondary px-2 py-3 text-center text-sm font-semibold tabular-nums !text-foreground transition-colors group-data-[focus-visible=true]:outline-2 group-data-[focus-visible=true]:outline-focus",
    timeContentSelected: "!bg-accent/10 !text-foreground",
    footer: "flex flex-wrap items-center justify-between gap-4 pt-4",
    footerSticky: "bg-surface",
