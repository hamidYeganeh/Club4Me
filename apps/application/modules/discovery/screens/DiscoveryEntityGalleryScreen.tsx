...
export function DiscoveryEntityGalleryScreen({
  type,
  id,
}: {
  type: "coach" | "class";
  ...
  const name =
    type === "coach" ? coach.data?.displayName : classItem.data?.title;
  const images =
    type === "coach"
  ...
  return (
    ...
    >
      <SecondaryHeader
        title={`گالری ${name}`}
        showFilter={false}
        backHref={`/discovery/${type === "coach" ? "coaches" : "classes"}/${id}`}
        action={
          <Button
            isIconOnly
            variant={gridVisible ? "primary" : "secondary"}
            aria-label={"تغییر نمای گالری"}
        ...
        <div className="grid flex-1 grid-cols-2 content-start gap-2 overflow-y-auto px-3 py-3">
          {slides.map((src, index) => (
            <button
              key={`${src}-${index}`}
              type="button"
              className="relative aspect-square overflow-hidden rounded-[1.35rem] bg-surface-secondary ring-accent data-[active=true]:ring-2"
          ...
          <div className="relative min-h-0 flex-1 py-3">
            ...
            {slides.length > 1 ? (
              ...
              >
                <Button
                  isIconOnly
                  className="pointer-events-auto rounded-full bg-foreground text-background"
                  aria-label="قبلی"
