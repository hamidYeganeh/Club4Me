...
const DIGITS = { latn: "0123456789", arabext: "۰۱۲۳۴۵۶۷۸۹" } as const;
type NumberingSystem = keyof typeof DIGITS;

const makeSizer = (digits: string) =>
  FACES.map((face) => (
...
const SIZERS = {
  latn: makeSizer(DIGITS.latn),
  arabext: makeSizer(DIGITS.arabext),
};
const STACKS = {
  latn: makeStack(DIGITS.latn),
  arabext: makeStack(DIGITS.arabext),
};
...
export function AnimatedCounter({
  ...
  const shift: Transition = reduced ? INSTANT : spring(shape.pace);
  ...
  return (
    ...
    >
      ...
      <span className="sr-only">
        ...
        {chars.replace(
          /\d/g,
          (digit) => DIGITS[numberingSystem][Number(digit)]!,
        )}
      </span>
