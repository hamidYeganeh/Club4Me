...
describe("coach attendance lifecycle", () => {
  ...
  it("requires arrival for departure, preserves timestamps on retry and clears stale times for absence", async () => {
    const attendance = connection.model(
      SessionAttendance.name,
      SessionAttendanceSchema,
    );
    const enrollments = connection.model(
      ClassEnrollment.name,
      ClassEnrollmentSchema,
    );
    const bookings = connection.model(
      SessionBooking.name,
      SessionBookingSchema,
    );
    await Promise.all([
      attendance.syncIndexes(),
      enrollments.syncIndexes(),
    ...
    await bookings.collection.insertOne({
    ...
    const service = new AttendanceService(
      ...
      bookings as never,
      { requireOwnedDocument: async () => session } as never,
      {
        findManyByIds: async () => [
          { id: String(athleteId), firstName: "علی" },
        ],
      } as never,
    );
    const present = {
    ...
    await expect(
      service.record(userId, String(sessionId), [
        { ...present, checkedOut: true },
      ]),
    ).rejects.toMatchObject({ code: "ATTENDANCE_CHECKOUT_REQUIRES_CHECKIN" });
    const arrived = (await service.record(userId, String(sessionId), [present]))
      .items[0]!;
    expect(arrived.checkedInAt).toBeTruthy();
    expect(arrived.athlete).toMatchObject({ firstName: "علی" });
    const exited = (
      await service.record(userId, String(sessionId), [
        { ...present, checkedOut: true },
      ])
    ).items[0]!;
    const retried = (
    ...
    expect(stored!.changes.map((change) => change.after)).toEqual([
      "present",
      "checked_out",
    ]);
    const absent = (
    ...
    await expect(
      service.record(userId, String(sessionId), [present]),
    ).rejects.toMatchObject({ code: "CLASS_SESSION_CANCELLED" });
  });
});
