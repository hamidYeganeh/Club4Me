...
import { PanelNavLinks } from "@repo/ui/panel-navigation";
  ...
  return (
    <>
        ...
        >
          <Drawer.Content placement="right">
            <Drawer.Dialog className={styles.drawerDialog()}>
              <Drawer.CloseTrigger aria-label={closeMenuLabel} />
              <Drawer.Header>
      ...
      <nav className="business-bottom-nav" aria-label="دسترسی سریع">
        {items
          .filter((item) =>
            ["/", "/check-in", "/students", "/calendar"].includes(item.href),
          )
          .map((item) => (
            <Link
              key={item.href}
