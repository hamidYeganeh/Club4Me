...
// Adapted from beui.dev/components/motion/tabs — theme tokens mapped to HeroUI.
...
export function Tabs({
  ...
  return (
    <MotionConfig reducedMotion={reduce ? "always" : "never"} transition={transition}>
      ...
      >
        ...
        {active ? (
          <motion.span
            ...
            layout="position"
            className={cn(
              "absolute -bottom-px inset-x-0 h-0.5 rounded-full bg-accent",
              indicatorClassName,
            )}
          />
        ) : null}
    ...
    <div className="relative">
      {active ? (
        <motion.span
          ...
          className={cn("absolute inset-0 bg-accent", radius, indicatorClassName)}
        />
      ) : null}
      <button
        type="button"
